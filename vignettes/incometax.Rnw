...
pay adjustment to argument \code{income} (viz. the penultimate example):~--

<<>>=
all.equal(incometax(125139, -10000), incometax(125139 + 10000), check.attributes = FALSE)
@

However, that equivalence may not always be
the~case:~--
...
Using this allowance, the additional amount of \emph{exactly} \pounds600 tax is recouped as
intended:~--
...
corresponding allowance rounded down\footnote{Rounding \emph{down} the Personal Allowance ensures
  recoup of a sum of tax \emph{not less than} that required.} to the nearest \pounds10 is obtained
using \code{allowance()} with its third argument \code{round10 = TRUE} (the~default):~--

<<>>=
allowance(10000, 600)
@

Using this allowance, the additional amount of \emph{at least} \pounds600 tax is recouped as
intended:~--
...
The Personal Allowance required to recoup a tax deficit of an \emph{additional} \pounds600 at this\\
income is:~--
...
\pagebreak
Using this allowance, the additional amount of \emph{at least} \pounds600 tax is recouped as
intended i.e., total income tax at least \pounds800:~--
...
attracts \pounds7,940 in income tax:~--

<<>>=
incometax(51279)
@

The Personal Allowance required to recoup a tax deficit of an \emph{additional} \pounds1,200 at this
income is:~--
...
Using this allowance, the additional amount of \emph{at least} \pounds1,200 tax is recouped as
intended i.e., total income tax at least \pounds9,140:~--
...
decreases by half, usually attracts \Sexpr{prettyL(sum(incometax(allow_upper + std_allow)))} in
income tax:~--
...
Using this pay adjustment, taxable income is adjusted to
\Sexpr{prettyL(attr(incometax(allow_upper + std_allow, -2460), "taxable"))} and the additional
amount of \emph{at least} \pounds3,500 tax is recouped as intended i.e., total income tax at
least \Sexpr{prettyL(sum(incometax(allow_upper + std_allow, -2460)))}:~--
