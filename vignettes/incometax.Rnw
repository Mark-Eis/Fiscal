...
\item annual taxable income after allowances above \Sexpr{prettyL(sumband)} attracts
  ...
  Allowance at or above \Sexpr{prettyL(allow_upper + std_allow * 2)}\footnote{In practice,
    an additional \pounds6 income is allowed before additional rate tax is applied; the additional
    \pounds6 is decreased by \pounds1 for every further \pounds2 increase in income beyond
    \Sexpr{prettyL(sumband + 7)}.}.
\end{itemize}
...
\pagebreak
\section{Income Above the Basic Rate Band}
...
The \emph{exact} Personal Allowance required to recoup a tax deficit may be calculated using
\code{allowance()} with its third argument \code{round10 = FALSE}; for example for a deficit of 
\pounds600 at an annual income of \pounds10,000 i.e., below the \Sexpr{prettyL(std_allow)} standard
Personal Allowance and not usually attracting any income tax:~--

<<>>=
allowance(10000, 600, FALSE)
@

Using this allowance, the intended additional amount of \emph{exactly} \pounds600 tax is
recouped:~--
...
In practice, this deficit would be recouped through use of tax code in which the last digit of the
required Personal Allowance is replaced with the letter  \textsf{L}, see
\href{https://www.gov.uk/tax-codes/what-your-tax-code-means}{What your tax code means} e.g., in the
previous example \textsf{699L}. The corresponding allowance rounded down\footnote{Rounding
  \emph{down} the Personal Allowance ensures recoup of a sum of tax \emph{not less than} that
  required.} to the nearest \pounds10 is obtained using \code{allowance()} with its third argument
\code{round10 = TRUE} (the~default):~--

<<>>=
allowance(10000, 600)
@

Using this allowance, the intended additional amount of \emph{at least} \pounds600 tax is
recouped:~--
...
\Sexpr{prettyL(std_allow)} standard Personal Allowance usually attracts \pounds200 in income tax:~--

<<>>=
incometax(13579)
@

To calculate the Personal Allowance required to recoup a tax deficit of an \emph{additional}
\pounds600 at this income:~--

<<>>=
allowance(13579, 200 + 600)
@

Using this allowance, the intended additional amount of \emph{at least} \pounds600 tax is recouped,
i.e., a total of at least \pounds800:~--

<<>>=
incometax(13579, 9570)
@

An annual income of \Sexpr{prettyL(std_allow + basicband + 1009)} i.e., \pounds1,009 above the basic
rate band, at the standard Personal Allowance, usually attracts \pounds7,940 in income tax:~--
...
To calculate the Personal Allowance required to recoup a tax deficit of an \emph{additional}
\pounds1,200 at this income:~--

<<>>=
allowance(51279, 7940 + 1200)
@

Using this allowance, the intended additional amount of \emph{at least} \pounds1,200 tax is
recouped, i.e., a total of at least \pounds9,140:~--
...
A tax deficit too great to recoup through reduction in Personal Allowance may instead be recouped
using a \href{https://www.gov.uk/tax-codes/k-in-your-tax-code}{K code pay adjustment}. For example,
an annual income of \Sexpr{prettyL(allow_upper + std_allow)}, at which there is half of the standard
Personal Allowance, usually attracts \Sexpr{prettyL(sum(incometax(allow_upper + std_allow)))} in
income tax:~--
...
To calculate the pay adjustment required to recoup a tax deficit of an \emph{additional}
\pounds3,500 at this income:~--

<<>>=
allowance(112570, 34970.4 + 3500)
@

Using this pay adjustment, adjusted taxable income is
\Sexpr{prettyL(attr(incometax(allow_upper + std_allow, -2460), "taxable"))}
at which the intended additional amount of \emph{at least} \pounds3,500 tax is recouped, i.e., a
total of at least \Sexpr{prettyL(sum(incometax(allow_upper + std_allow, -2460)))}:~--
